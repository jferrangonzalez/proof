...
class Template3 extends BaseTemplate
{
    ...
    public function addInvoiceLines($model)
    {
        ...
        if (!empty($this->getObservations($model))) {
            $this->writeHTML('<p class="p3"><b>' . $this->toolBox()->i18n()->trans('observations') . '</b><br/>' . $this->getObservations($model) . '</p>');
        }
        ...
        $clonedPdf->writeHTML($this->getInvoiceTotals($model));
        ...
        $this->writeHTML($this->getInvoiceTotals($model));
        ...
        foreach ($fields as $key => $title) {
            ...
            switch ($key) {
                ...
                case 'dtopor2':
                    ...
                    $tds .= '<td align="center">' . $numbers->format($model->{$key}) . '%</td>';
                    break;
